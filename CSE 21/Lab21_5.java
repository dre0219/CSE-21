...
public class Lab21_5 {
		...
		public static long sumAll(int max) {
			...
			long subTotal = 0;
			for (int i = 1; i <= max; i++) {
				subTotal += i; // adds i to the the sub total and adds every time after each run
		...
		public static long sumAll(long[] arr, int max) {
			...
			for (int i = 1; i <= max ; i++) {
				if (arr[i] == 0) { //start the array from 0
					arr[i] = arr[i-1] + i;
			...
			long squareSum = 0;
				...
				public static long fib(long[] arr, int max) {
			        ...
			        for (int i = 2; i <= max; i++) {
			            factorial *= i;
			        System.out.println("Fatorial " + i + " value " + factorial);
			        }
